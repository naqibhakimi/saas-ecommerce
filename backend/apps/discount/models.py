...
class DiscountCondition(BaseModel):
    ...
    type = models.CharField(max_length=20, choices=Discount_Condition_Type)
    operator = models.CharField(max_length=20, choices=Discount_Condition_Operator)
    discount_rule = models.ForeignKey(
        "DiscountRule", on_delete=models.CASCADE, related_name="+"
    )
...
class Discount(BaseModel):
    ...
    rule = models.ForeignKey(
        DiscountRule, on_delete=models.CASCADE, null=True, related_name="+"
    )
    is_disabled = models.BooleanField()
    parent_discount = models.ForeignKey(
...
class DiscountConditionCustomerGroup(BaseModel):
    customer_group = models.ForeignKey(
        "customer.CustomerGroup", on_delete=models.CASCADE
    )
...
class DiscountConditionProductTag(BaseModel):
    ...
    discount_condition = models.ForeignKey(
        DiscountCondition, on_delete=models.CASCADE, related_name="+"
    )
    metadata = models.JSONField(null=True, blank=True)
